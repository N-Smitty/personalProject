const breeding = (type, ngMl, res) => {
  if (type === "Artificial Insemination") {
    if (ngMl < 2) {
      ...
      res
        .status(200)
        .send(
          "This method will no longer be effective. Consider trans-cervical or surgical insemination methods."
        );
    }
  } else if (type === "Trans-cervical Insemination") {
    if (ngMl < 2.7) {
      ...
      res
        .status(200)
        .send(
          "This method will no longer be effective. Consider Surgical Insemination."
        );
    }
  } else if (type === "Surgical Insemination") {
    if (ngMl < 3) {
      res.status(200).send("Re-test in 4 days");
      ...
      res
        .status(200)
        .send(
          "This method will no longer be effective. Try again during the next heat cycle."
        );
    }
  }
};
